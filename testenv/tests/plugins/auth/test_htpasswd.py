...
async def test_ok__htpasswd_service(tmpdir) -> None:  # type: ignore
    ...
    async with get_configured_auth_service("htpasswd", file=path) as service:
        ...
        assert not (await service.authorize("", ""))
        assert not (await service.authorize("admin@", "pass"))
        assert not (await service.authorize("admin@example", "pass"))
        assert not (await service.authorize("admin@example.", "pass"))
        assert not (await service.authorize("admin@example.com", "pass"))
        assert (await service.authorize("admin", "pass"))

        htpasswd.set_password("admin", "bar")
        htpasswd.set_password("user", "baz")
        htpasswd.set_password("admin@example.com", "x")
        htpasswd.save()

        assert (await service.authorize("admin", "bar"))
        assert (await service.authorize("user", "baz"))
        assert (await service.authorize("admin@example.com", "x"))
        assert not (await service.authorize("admin", "foo"))
        assert not (await service.authorize("user", "foo"))

        assert not (await service.authorize("admin@", "x"))
        assert not (await service.authorize("admin@example", "x"))
        assert not (await service.authorize("admin@example.", "x"))
